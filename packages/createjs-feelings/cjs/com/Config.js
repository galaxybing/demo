...
{// cjs/com/Core.js  this.addContainer -> Config.resize([obj]);
    var w,h;
...
Config.preloading=function(arr)
{
    ...
    if( tmpStr.indexOf('digg.tgideas.qq.com/')==-1 &&tmpStr.indexOf('qq.com/')!=-1 ){
    ...
    */
    ...
    function movieJsHandleComplete(e)
    {
        ...
        for(var i=0;i<arr[currentPage][4].length;i++)
        {
            ...
            {
                arr[currentPage][4][i].src=arr[currentPage][4][i].src;
            }
            else
            {
                arr[currentPage][4][i].src=arr[currentPage][4][i].src+'?rnd='+String(Math.random()*100>>0);
            }
        ...
        {
            tmpArr[i].src=tmpArr[i].src+'?rnd='+String(Math.random()*100>>0);
        }
    ...
    function handleFileLoad(evt) {
        ...
        if (evt.item.type == "image") {
            ...
            //console.log('evt.result:', arr[currentPage][2]);
        }
    }
...
Config.addFps=function(target,num)
{
    ...
    //var graphics = new createjs.Graphics().beginFill("#000000").drawRect(0, 0, 180, 38);
    var graphics = new createjs.Graphics().beginFill("#000000").drawRect(0, 0, 180, 38);
...
//添加移动端设备打印信息文本;
Config.addOutput=function(target)
{
    //var txt=new cjs.Text('\r\r/**output content**/','900 20px Arial','#0');
    var txt=new cjs.Text('\r\r/**output content**/','900 20px Arial','#ffffff');
    txt.mouseEnabled=false;
    target.addChild(txt);
    Config.outputTxt=txt;
}
//在移动端设备打印信息文本中输出字符串内容;
Config.trace=function(str)
{
    if(!Config.isTest) return;
    var txtStr=Config.outputTxt.text;
    var arr=txtStr.split('\r');
    var showNum=35;
    var tmp='';
    var num=0;
    if(arr.length>showNum) num=arr.length-showNum;
    for(var i=num;i<arr.length;i++)
    {
        tmp+='\r'+arr[i];
    }
    //console.log(arr.length);
    Config.outputTxt.text=tmp;
    Config.outputTxt.text+='\r'+String(str);

}

Config.addAudio=function(id,src,autoplay,loop,isBgSound)
{
    var audio = document.createElement("audio");
    audio.id=id;
    //console.log('addAudio:',audio.id);
    audio.onplaying = function()
    {

    };
    audio.autoplay = autoplay;// 自动播放 1) html5 标准属性配置
    audio.loop = loop;
    if (audio.canPlayType("audio/mpeg")) {
        audio.type= "audio/mpeg";
        audio.src = Config.path+src + ".mp3?rnd="+String(Math.random()*100>>0);
        //audio.src = Config.path+src + ".mp3";
        //alert('audio.src:'+audio.src);
        //console.log('audio.src:'+audio.src);

    } else if(audio.canPlayType("audio/ogg")){
        audio.type= "audio/ogg";
        audio.src = Config.path+src + ".ogg"
    }
    if(autoplay){
        Config.autoplayAudio(audio, isBgSound);
...
Config.autoplayAudio= function(audio){

    document.body.appendChild(audio);
    var flag = 0;
    if(Config.isAndroid()){// android 版本微信，不存在禁用视、音频自动播放的限制
        return;
    }

    if(!Config.isWeixin()){// 非 android 环境，非微信下也直接退出
        if(flag == 0 && audio.paused){
            //document.addEventListener('touchstart', autoPlay);
            Config.CANVAS.addEventListener('touchstart', autoPlay);
            function autoPlay() {
                if(flag == 0 && audio.paused){
                    //audio.load();
                    audio.play();
                    //document.removeEventListener('touchstart',autoPlay);
                    Config.CANVAS.removeEventListener('touchstart', autoPlay);
    ...
    postAjax("http://juzhen.zjqq.mobi/index.php/jssdk/index?url=" + window.location.href, {}, function (data) {
        wx.config({
            debug: false,
            appId: data.appId,
    ...
    /*需要配置在 new Main 实例化之后， Config.CANVAS.
    if(isBgSound)
    {
