...
function Core() {
    ...
    this.setSource = function (arr) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            data[i][3] = arr[i].id;
            data[i][4] = [];
            for (var j = 0; j < arr[i].atlas_P; j++) {
                var tmp = String(j + 1);
                if (tmp == '1') tmp = '';
                data[i][4].push({
                    src: Core.spritesheetPath + arr[i].id + '/' + arr[i].id + '_atlas_P_' + tmp + '.json',
                    type: 'spritesheet',
                    //type: 'FlashSprite',
                    id: arr[i].id + '_atlas_P_' + tmp
                });
            }
    ...
    this.getMc=function(i)//获取对应的场景动画内容;
    {
        ...
        else if(arr.mc==undefined)//当mc加载完成,但为组成mc;
        {
            ...
            mc.pageId=i;
            children0=mc.instance;
            //console.log(mc)
            //console.log('mc.pageId:',mc.pageId);
            if(children0.totalFrames)
            {
                target.addFrameScript(children0,children0.totalFrames-1,function(mc)
                {
                    mc.stop();
    ...
    this.addScenes=function(name,id)//播放场景;
    {
        ...
        if(this.getContainer(name).numChildren==0)//首个场景,无需退场,开始当前场景进场动作;
        {
            ...
            mc.instance.addEventListener('added',function(e){
                setTimeout(function(){
                    target.onScenesIn(mc);
                },50);
            })
        }
    ...
    //由外部触发;
    this.onScenesNextLoadComplete=function(name)//下个场景是否资源加载完成,但此时尚未组成mc,组成mc需要耗费较大内存;
    {
        console.log('onScenesNextLoadComplete:',tmpId);
        if(mcDataArr[tmpId])//如果下一个场景已经加载完毕,则执行下个场景的开场;
        {
            target.onScenesRead(tmpId);
        }
        else//如果下一个场景尚未加载完成,则不执行;
        {
...
coreP.onScenesRead=function(pageId)//场景素材加载完成,但为进行mc组合;
{
